...
final class TaskWebViewController: UIViewController, WKNavigationDelegate, WKUIDelegate {
    ...
    private func buildWebView() {
        ...
        injectConsoleCapture(into: config.userContentController)
    ...
    private func setupBridgeCallbacks() {
        consoleBridge.onConsoleMessage = { level, message in
            let line = "WebView console [\(level)] \(message)"
            switch level {
            case "error":
                DataPointLogger.e(line)
            case "warn":
                DataPointLogger.w(line)
            default:
                DataPointLogger.d(line)
            }
        }
    ...
    /// Forwards `console.log` / `warn` / `error` / `info` / `debug` to native `DataPointLogger` (debug builds).
    private func injectConsoleCapture(into ucc: WKUserContentController) {
        let bridge = SdkConstants.jsBridgeConsole
        let js = """
        (function() {
            var bridgeName = '\(bridge)';
            if (!window.webkit || !window.webkit.messageHandlers || !window.webkit.messageHandlers[bridgeName]) { return; }
            ...
            function post(level, args) {
                try {
                    ...
                    window.webkit.messageHandlers[bridgeName].postMessage({ level: level, message: message });
                } catch (e) {}
            }
