...
public enum DataPoint {
    ...
    public static func initialize(
        ...
        if environment == .sandbox {
            handleSandboxInit(prefs: prefs, callback: callback)
            return
        }
    ...
    public static func showTasks(from presentingViewController: UIViewController) {
        ...
        if environment == .production && !prefs.isSessionValid() {
    ...
    private static func handleSandboxInit(prefs: DataPointPreferences, callback: InitCallback?) {
        DataPointLogger.d("Sandbox mode – using mock session")
        prefs.sessionToken = "sandbox_token_\(Int(Date().timeIntervalSince1970 * 1000))"
        prefs.sessionExpiry = Date().timeIntervalSince1970 + 86_400
        prefs.userId = userId ?? "sandbox_user_\(String(prefs.deviceId.prefix(8)))"
        prefs.apiKey = apiKey
        transitionToInitialized()
        postOnMain { callback?.onSuccess() }
    }
